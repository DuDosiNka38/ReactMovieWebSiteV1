...
class LockScreenForm extends Component {
  ...
  signIn = async (postData) => {
    if(postData === undefined){
      ...
      if(user_email.indexOf("@") === -1){
        ...
        const result = await axios.get(`/api/user/Person_id/${user_email}`).then((r) => (r.data)).catch((e) => (false));
...
export default connect(mapStateToProps, mapDispatchToProps)(LockScreenForm);
